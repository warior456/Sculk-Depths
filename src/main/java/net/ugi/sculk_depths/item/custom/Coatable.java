...
public interface Coatable {
    Supplier<BiMap<Block, Block>> COAT_LEVEL_INCREASES = Suppliers.memoize(() -> ImmutableBiMap.<Block, Block>builder()
    ...
    static void coatBlock(BlockState state, World world, BlockPos pos) {
        ...
        if (block == ModBlocks.VALTROX_DOOR) {
            world.setBlockState(pos, coatedblock.getStateWithProperties(state.with(DoorBlock.HALF, DoubleBlockHalf.LOWER)));
            world.setBlockState(pos.up(), coatedblock.getStateWithProperties(state.with(DoorBlock.HALF, DoubleBlockHalf.UPPER)));
        }
