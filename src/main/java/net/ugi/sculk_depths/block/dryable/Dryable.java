...
public interface Dryable {
    Supplier<BiMap<Block, Block>> DRY_LEVEL_INCREASES = Suppliers.memoize(() -> ImmutableBiMap.<Block, Block>builder()
    ...
    static void dryBlock(BlockState state, ServerWorld world, BlockPos pos) {
        ...
        if (block == ModBlocks.VALTROX_DOOR) {
            world.setBlockState(pos, driedblock.getStateWithProperties(state.with(DoorBlock.HALF, DoubleBlockHalf.LOWER)));
            world.setBlockState(pos.up(), driedblock.getStateWithProperties(state.with(DoorBlock.HALF, DoubleBlockHalf.UPPER)));
        }
